...
module.exports = function defineCustomHook(sails) {
  ...
  return {
    ...
    initialize: async function () {
      ...
      var IMPORTANT_AVIMESA_CONFIG = ['avimesaHostname', 'avimesaApiKey', 'avimesaApiPassword'];
      var isMissingAvimesaConfig = _.difference(IMPORTANT_AVIMESA_CONFIG, Object.keys(sails.config.custom)).length > 0;
      if(isMissingAvimesaConfig) {
        throw new Error(`Missing Avimesa config!\nIn order to run this app, you must first configure the following Avimesa API credentials:\n• sails.config.custom.avimesaHostname\n• sails.config.custom.avimesaApiKey\n• sails.config.custom.avimesaApiPassword\n(Need an API key? Visit https://avimesa.com/request-developer-account/)`);
      }
...
---------------------------------------------------------------------${suffix}`);
      ...
      sails.after('hook:organics:loaded', ()=>{
        ...
        sails.helpers.avimesa.configure({
        ...
        '/*': {
          ...
          fn: async function(req, res, next){
            ...
            if (req.method === 'GET') {
              ...
              // make the platform information from custom config available via Sails locals
              res.locals.platformName = sails.config.custom.platformName;
